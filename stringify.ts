...
export function stringify(object, opts?: StringifyOptions) {
  ...
  const delimiter =
    typeof options.delimiter === 'undefined' ? defaults.delimiter : options.delimiter
  ...
  const serializeDate =
    typeof options.serializeDate === 'function' ? options.serializeDate : defaults.serializeDate
