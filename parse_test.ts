...
t('parses a simple string', function() {
  ...
  assertEqual(parse('foo=bar&baz', { strictNullHandling: true }), { foo: 'bar', baz: null })
  assertEqual(parse('foo=bar&baz'), { foo: 'bar', baz: '' })
...
assertEqual(parse('a[b]=c'), { a: { b: 'c' } }, 'parses a single nested string')
assertEqual(parse('a[b][c]=d'), { a: { b: { c: 'd' } } }, 'parses a double nested string')
assertEqual(
...
t('only parses one level when depth = 1', function() {
  ...
  assertEqual(parse('a[b][c][d]=e', { depth: 1 }), { a: { b: { '[c][d]': 'e' } } })
})
...
assertEqual(parse('a[12b]=c'), { a: { '12b': 'c' } }, 'supports keys that begin with a number')
...
t('transforms arrays to objects', function() {
  assertEqual(parse('foo[0]=bar&foo[bad]=baz'), { foo: { 0: 'bar', bad: 'baz' } })
  assertEqual(parse('foo[bad]=baz&foo[0]=bar'), { foo: { bad: 'baz', 0: 'bar' } })
  assertEqual(parse('foo[bad]=baz&foo[]=bar'), { foo: { bad: 'baz', 0: 'bar' } })
  assertEqual(parse('foo[]=bar&foo[bad]=baz'), { foo: { 0: 'bar', bad: 'baz' } })
  ...
  assertEqual(parse('a[]=b&a[t]=u&a[hasOwnProperty]=c', { allowPrototypes: false }), {
    a: { 0: 'b', t: 'u' }
  })
  assertEqual(parse('a[]=b&a[t]=u&a[hasOwnProperty]=c', { allowPrototypes: true }), {
    a: { 0: 'b', t: 'u', hasOwnProperty: 'c' }
  })
  assertEqual(parse('a[]=b&a[hasOwnProperty]=c&a[x]=y', { allowPrototypes: false }), {
    a: { 0: 'b', x: 'y' }
  })
  assertEqual(parse('a[]=b&a[hasOwnProperty]=c&a[x]=y', { allowPrototypes: true }), {
...
t('transforms arrays to objects (dot notation)', function() {
  ...
  assertEqual(parse('foo[0].baz[0]=15&foo[0].baz[1]=16&foo[0].bar=2', { allowDots: true }), {
    foo: [{ baz: ['15', '16'], bar: '2' }]
  })
  ...
  assertEqual(parse('foo[0].a=a&foo[0].b=b&foo[1].a=aa&foo[1].b=bb', { allowDots: true }), {
    foo: [{ a: 'a', b: 'b' }, { a: 'aa', b: 'bb' }]
  })
})

t('correctly prunes undefined values when converting an array to an object', function() {
  assertEqual(parse('a[2]=b&a[99999999]=c'), { a: { 2: 'b', 99999999: 'c' } })
})
...
t('compacts sparse arrays', function() {
  ...
  assertEqual(parse('a[1][b][2][c]=1', { arrayLimit: 20 }), { a: [{ b: [{ c: '1' }] }] })
  assertEqual(parse('a[1][2][3][c]=1', { arrayLimit: 20 }), { a: [[[{ c: '1' }]]] })
  assertEqual(parse('a[1][2][3][c][1]=1', { arrayLimit: 20 }), { a: [[[{ c: ['1'] }]]] })
})
...
t('continues parsing when no parent is found', function() {
  ...
  assertEqual(parse('[]&a=b', { strictNullHandling: true }), { 0: null, a: 'b' })
  assertEqual(parse('[foo]=bar'), { foo: 'bar' })
})
...
t('should not throw when a native prototype has an enumerable property', function() {
...
t('allows setting the parameter limit to Infinity', function() {
  assertEqual(parse('a=b&c=d', { parameterLimit: Infinity }), { a: 'b', c: 'd' })
})
...
t('allows overriding array limit', function() {
  ...
  assertEqual(parse('a[0]=b&a[1]=c', { arrayLimit: 0 }), { a: { 0: 'b', 1: 'c' } })
})

t('allows disabling array parsing', function() {
  const indices = parse('a[0]=b&a[1]=c', { parseArrays: false })
  assertEqual(indices, { a: { 0: 'b', 1: 'c' } })
  assertEqual(Array.isArray(indices.a), false, 'parseArrays:false, indices case is not an array')
...
t('allows for query string prefix', function() {
  ...
  assertEqual(parse('foo=bar', { ignoreQueryPrefix: true }), { foo: 'bar' })
  assertEqual(parse('?foo=bar', { ignoreQueryPrefix: false }), { '?foo': 'bar' })
})
...
t('does not allow overwriting prototype properties', function() {
  ...
  assertEqual(parse('toString', { allowPrototypes: false }), {}, 'bare "toString" results in {}')
})
...
t('can allow overwriting prototype properties', function() {
  ...
  assertEqual(parse('hasOwnProperty=b', { allowPrototypes: true }), { hasOwnProperty: 'b' })
...
t('add keys to objects', function() {
  assertEqual(parse('a[b]=c&a=d'), { a: { b: 'c', d: true } }, 'can add keys to objects')

  assertEqual(parse('a[b]=c&a=toString'), { a: { b: 'c' } }, 'can not overwrite prototype')
...
t('can return null objects', function() {
  ...
  assertEqual(parse('a[b]=c&a[hasOwnProperty]=d', { plainObjects: true }), expected)
...
t('uses the utf8 sentinel to switch to utf-8 when no default charset is given', function() {
  assertEqual(
    parse(
      'utf8=' +
        urlEncodedCheckmarkInUtf8 +
...
t('uses the utf8 sentinel to switch to iso-8859-1 when no default charset is given', function() {
  assertEqual(
    parse(
      'utf8=' +
        urlEncodedNumCheckmark +
...
t('interprets numeric entities in iso-8859-1 when `interpretNumericEntities`', function() {
  assertEqual(
    parse('foo=' + urlEncodedNumSmiley, { charset: 'iso-8859-1', interpretNumericEntities: true }),
    { foo: '☺' }
  )
})
...
t(
  ...
  function() {
    assertEqual(parse('foo=' + urlEncodedNumSmiley, { charset: 'iso-8859-1' }), { foo: '&#9786;' })
  }
)
...
t('does not interpret %uXXXX syntax in iso-8859-1 mode', function() {
  assertEqual(parse('%u263A=%u263A', { charset: 'iso-8859-1' }), { '%u263A': '%u263A' })
})
