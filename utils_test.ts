...
t('merge()', function() {
  ...
  const twoMerged = utils.merge({ foo: ['bar', { first: '123' }] }, { foo: { second: '456' } })
  assertEqual(
    twoMerged,
  ...
  const sandwiched = utils.merge({ foo: ['bar', { first: '123', second: '456' }] }, { foo: 'baz' })
  assertEqual(
    sandwiched,
...
t('one array, one non-array', function() {
  ...
  assertEqual([1, 2], combinedAnB, 'first argument is array-wrapped when not an array')
  ...
  assertEqual([1, 2], combinedABn, 'second argument is array-wrapped when not an array')
})
...
t('neither is an array', function() {
  ...
  assertEqual([1, 2], combined, 'both arguments are array-wrapped when not an array')
})
